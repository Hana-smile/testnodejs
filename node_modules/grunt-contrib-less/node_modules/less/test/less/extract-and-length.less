...
.multiunit { @
	...
	length: length(@ v);
	extract: extract(@ v, 1) extract(@ v, 2) extract(@ v, 3) extract(@ v, 4)
		extract(@ v, 5) extract(@ v, 6);
}
...
.mixin-arguments { .mixin-args (a b c d); .mixin-args (a, b, c, d); .mixin-args
	(1; 2; 3; 4);
...
.mixin-args (@value) { &-1 {
	length: length(@ value);
	extract: extract(@ value, 3)~"|" extract(@ value, 2)~"|"
		extract(@ value, 1);
}
...
.mixin-args (...) { &-2 {
	...
	extract: extract(@ arguments, 3)~"|" extract(@ arguments, 2)~"|"
		extract(@ arguments, 1);
}
...
.mixin-args (@values...) { &-3 {
	...
	extract: extract(@ values, 3)~"|" extract(@ values, 2)~"|"
		extract(@ values, 1);
}

}
.mixin-args (@head, @tail ...) { &-4 {
	length: length(@ tail);
...
.md-3D { @
	a: a b c d, 1 2 3 4; @
	b: 5 6 7 8, e f g h; .3 D (@a, @b); .3 D (...) { @v1 : @ arguments;
	length-1: length(@ v1);
