...
.equality-units { .equality-unit-test (1px); .equality-unit-test (2px);
...
.colorguardtest { .colorguard (red); .colorguard (blue); .colorguard
	(purple);
...
.stringguardtest { .stringguard ("theme1"); .stringguard ("theme2"); .stringguard
	('theme1'); .stringguard ('theme2'); .stringguard (theme1);
...
.call-lock-mixin { .lock-mixin (1); .call-inner-lock-mixin {
	.inner-locked-mixin();
	
}

}
.bug-100cm-1m (@a) when  (@a  = 1) { .failed {
...
#ns { .mixin-for-root-usage (@a) when (@a > 0) { .mixin-generated-class
	{ a: @a;
