...
#built-in { @
	...
	sqrt: sqrt(25px);
	pi: pi();
	mod: mod(13m, 11cm); // could take into account units, doesn't at the
	moment abs : abs( -4%);
	tan: tan(42deg);
	sin: sin(10deg);
	cos: cos(12);
	...
	fade-out: fadeOut(red, 5%); // support fadeOut and fadeout fade-in :
	fadein( fadeout( red, 10%), 5%);
	hsv: hsv(5, 50%, 30%);
	...
	mixt: mix(#ff0000, transparent); . is-a { color : iscolor( #ddd);
	color1: iscolor(red);
...
#extract-and-length { @
	anon: A B C 1 2 3;
	extract: extract(@ anon, 6) extract(@ anon, 5) extract(@ anon, 4)
		extract(@ anon, 3) extract(@ anon, 2) extract(@ anon, 1);
	length: length(@ anon);
}